  ...
  def create_penalties_for_yellow_cards(yellows)
    yellows.each do |player_id, cards|
      if cards > 1
        ...
        incidences = Incidence.incidences_for_player_id_and_incidence_type(player_id, 1).order(:match_id)
  ...
  def create_penalties_for_red_cards(reds)
    reds.each do |player_id, cards|
      if cards > 0
        ...
        # Deactivate Incidences for this Player
        incidences = Incidence.incidences_for_player_id_and_incidence_type(player_id, 2).order(:match_id)
        penalty = Penalty.create(
          player_id: player_id,
          match_id: incidences.last.match_id,
